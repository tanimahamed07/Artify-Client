            ...
            <div className="mb-6 text-center space-x-4">
                {buttons.map(cat => (
                    <button
                        ...
                        className={`px-4 py-2 rounded font-medium ${category === cat ? 'bg-[#FF6F61] text-white' : 'bg-gray-200 text-gray-700'
                ...
                {category && (
                    <button
                        onClick={() => setCategory('')}
                        className={`px-4 py-2 rounded font-medium ${category === '' ? 'bg-[#FF6F61] text-white' : 'bg-gray-200 text-gray-700'
                            } hover:bg-[#FF6F61] hover:text-white transition`}
                    >
                        All
                    </button>
                ...
                <div className="mb-10">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                        {filteredArtworks.length > 0 ? (
                            filteredArtworks.map(art => <ArtsCard key={art._id} art={art} />)
                        ) : (
                ...
                <div className="mb-10">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                        {categoryFilteredArtworks.length > 0 ? (
                            categoryFilteredArtworks.map(art => <ArtsCard key={art._id} art={art} />)
                        ) : (
